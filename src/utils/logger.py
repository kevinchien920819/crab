...
class RelativePathFormatter(logging.Formatter):
    def __init__(self, fmt=None, datefmt=None, project_root=None):
        """Initialize a formatter that records paths relative to the project root."""
        super().__init__(fmt, datefmt)
        self.project_root = project_root or os.getcwd()

    def format(self, record):
        """Attach a relative_path attribute before formatting a log record."""
...
def setup_logger(name, project_root, log_file=None, level=logging.INFO) -> logging.Logger:
    """Create a console and optional file logger with relative-path formatting."""
